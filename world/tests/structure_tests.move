...
use world::is_proximity::{Self, IsProximity};
...
fun transport_can_be_attached_to_structure() {
    ...
    transport::add_default_requirement<Jump>(&mut s, &acl, is_proximity::requirement(), sc.ctx());
    ts::return_shared(s);
    ...
    is_proximity::verify_proximity(&mut req, sc.ctx());
